...
static void* ruea_open(const char* path) { return dlopen(path, RTLD_NOW); }
static void ruea_close(void* h) { if (h) dlclose(h); }
static void* ruea_sym(void* h, const char* name) { return dlsym(h, name); }

static int ruea_bind(void* h, ruea_binding* out) {
...
func openPlatformPlugin(path string) (*platformPlugin, error) {
	...
	h := C.ruea_open(cpath)
	if h == nil {
		return nil, fmt.Errorf("dlopen failed for %s", path)
	}
